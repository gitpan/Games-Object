...
The current version is 0.04 (alpha). The following are the changes from last
version:
...
later versions of this module. Either the processing model will be adjusted,
or a means of detecting when mods are added from these events and performing
the magic behind the scenes to fix the problem will be devised.
...
E<lt>stwrtpj@earthlink.netE<gt> with a description of the bug.
...
E<lt>stwrtpj@earthlink.netE<gt>. Also feel free to drop me a comment if
